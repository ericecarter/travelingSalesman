...
public class InClassHeuristic {
	...
	private Node uniformCostSearch(Node root) {
		...
		frontierGenerator.updateFrontierCross(frontier, root);
		Node current;
		...
		do {
			if (frontier.isEmpty())
				...
				if (current.allVisited()) {
					...
					frontierGenerator.updateFrontierCross(frontier, current);
				}
			}
		} while (true);
	}
